...
class Graph:
    def check(self, n, m, L, st, x):
        s, t = tuple(map(int, st.split(' ')))
        graph = GraphImplementation(n)
        missing_graph = GraphImplementation(n)

        for edge in x:
            i, j, w = map(int, edge.split(' '))
            graph.add_edge_undirected(i, j, w)

            if w:
                missing_graph.add_edge_undirected(i, j, w)
            else:
                missing_graph.add_edge_undirected(i, j, 2 ** 63)

        if graph.dijkstra(s)[t] <= L <= missing_graph.dijkstra(s)[t]:
            return 'YES'
        else:
            return 'NO'
