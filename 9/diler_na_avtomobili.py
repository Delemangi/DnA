...
class Diler:
    def docnenje(self, N, M, X, Y):
        graph = HashGraph(N + M + 2)
        s = N + M
        t = N + M + 1

        for i, (x, y) in enumerate(zip(X, Y.split(' '))):
            r, *cars = x.split(' ')

            for j, p in enumerate(cars):
                graph.add_edge(i, N + j, int(p))

            graph.add_edge(s, i, int(r))
            graph.add_edge(N + i, t, int(y))
