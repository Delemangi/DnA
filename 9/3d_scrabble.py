...
class Scrabble:
    def indexes(self, N, M):
        n = len(N.split(' '))
        m = len(M.split(' '))
        output = []

        for i, word in enumerate(M.split(' ')):
            w = len(word)
            src = w + m
            dest = w + m + 1
            graph = HashGraph(w + m + 2)

            for c, ch in enumerate(word):
                for j, chars in enumerate(N.split(' ')):
                    if ch in chars:
                        graph.add_edge(c, n + j, 1)

                graph.add_edge(src, c, 1)

            for j, chars in enumerate(N.split(' ')):
                graph.add_edge(n + j, dest, 1)

            if graph.ford_fulkerson(src, dest) == len(word):
                output.append(str(i))
