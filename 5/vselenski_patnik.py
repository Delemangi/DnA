...
class VselenskiPatnik:
    def pat(self, N, M, x):
        graph = Graph(N)
        planets = [i - 1 for i in map(int, x.split(' '))]

        for i in range(N):
            for j in range(N):
                graph.add_edge_directed(i, j, int(M[i].split(' ')[j]))

        space_map = graph.floyd_warshall()

        for i in range(N):
            if space_map[i][i] < 0:
                return 'Se izgubi vo vremeto'

        current_planet = planets[0]
        total = 0

        for planet in planets[1:]:
            total += space_map[current_planet][planet]
